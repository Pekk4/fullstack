...
before(async () => {
  ...
  // Create session user
  sessionUser = await createUser('session@example.com', 'Session User');
...
describe('Reading Lists API', () => {
  ...
  it('user can view their reading list', async () => {
    ...
    assert.ok(reading.readinglists);
    assert.strictEqual(typeof reading.readinglists.read, 'boolean');
  });
  ...
  it('user can mark a blog as read with authentication', async () => {
    ...
    const readingListId = userResponse.data.readings[0].readinglists.id;
